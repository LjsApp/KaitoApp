    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-border bg-card overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-10 text-center text-muted-foreground text-sm">Belum ada pesan yang masuk.</div>
        ) : (
          <ul className="divide-y divide-border">
            {messages.map((m) => (
              <li key={m.id} className={`p-4 hover:bg-accent/40 transition-colors ${!m.read ? "bg-primary/5" : ""}`}>
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <button onClick={() => { setOpen(m); if (!m.read) toggleRead(m); }} className="flex-1 min-w-0 text-left">
                    <div className="flex items-center gap-2">
                      {!m.read && <span className="h-2 w-2 rounded-full bg-accent-orange shrink-0" />}
                  ...
                  <div className="flex gap-1 shrink-0">
                    <Button size="sm" variant="ghost" onClick={() => toggleRead(m)} title={m.read ? "Tandai belum dibaca" : "Tandai sudah dibaca"}>
                      {m.read ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
                    </Button>
          ...
          <div className="bg-card rounded-2xl border border-border shadow-elegant max-w-2xl w-full">
            <div className="border-b border-border px-6 py-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">{new Date(open.created_at).toLocaleString("id-ID")}</p>
              </div>
              <button onClick={() => setOpen(null)} className="grid h-8 w-8 place-items-center rounded-lg hover:bg-accent"><X className="h-4 w-4" /></button>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div>
                <div className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-2">Pesan</div>
                <div className="rounded-xl bg-muted/50 p-4 text-sm whitespace-pre-wrap leading-relaxed">{open.message}</div>
              </div>
              <div className="flex flex-wrap gap-2 pt-2 border-t border-border">
                <a href={`mailto:${open.email}`} className="inline-flex"><Button size="sm" variant="outline"><Mail className="h-4 w-4 mr-2" /> Balas Email</Button></a>
                <a href={`https://wa.me/${open.phone.replace(/\D/g, "")}`} target="_blank" rel="noopener noreferrer" className="inline-flex">
                  <Button size="sm" variant="outline"><Phone className="h-4 w-4 mr-2" /> WhatsApp</Button>
                </a>
                <Button size="sm" variant="ghost" onClick={() => remove(open.id)} className="text-destructive hover:text-destructive ml-auto">
