...
type ArtForm = {
  ...
  category: string; cover_url: string; content: string; published_at: string;
};

const EMPTY: ArtForm = {
  slug: "", title: "", excerpt: "", category: "Panduan",
  cover_url: "", content: "", published_at: new Date().toISOString().slice(0, 10),
};
    ...
    <Modal title={isNew ? "Tambah Artikel" : `Edit: ${form.title}`} onClose={onClose} wide>
      <div className="space-y-5">
        <div className="grid sm:grid-cols-2 gap-4">
          <Input label="Judul Artikel" value={form.title} onChange={updateTitle} required className="sm:col-span-2" />
          <Input label="Slug (URL)" value={form.slug} onChange={(v) => set("slug", v)} placeholder="otomatis dari judul" />
          <div>
          ...
          <Input label="Tanggal Publish" type="date" value={form.published_at} onChange={(v) => set("published_at", v)} />
          <Textarea label="Ringkasan (Excerpt)" value={form.excerpt} onChange={(v) => set("excerpt", v)} className="sm:col-span-2" rows={3} />
        </div>

        <ImageUpload label="Foto Cover Artikel" value={form.cover_url} onChange={(v) => set("cover_url", v)} folder="articles" maxSizeMB={1} />

        <RichTextEditor label="Isi Artikel" value={form.content} onChange={(v) => set("content", v)} />
      </div>

      <div className="flex justify-end gap-2 mt-6 pt-4 border-t border-border">
        <Button variant="outline" onClick={onClose}>Batal</Button>
        <Button onClick={save} disabled={loading}>{loading ? "Menyimpan..." : "Simpan Artikel"}</Button>
      </div>
    </Modal>
  );
}
...
function AdminArticlesPage() {
  ...
  const openEdit = (a: typeof articles[number]) => setEditing({
    id: a.id,
    slug: a.slug || "",
    title: a.title || "",
    excerpt: a.excerpt || "",
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Button onClick={openNew}><Plus className="h-4 w-4 mr-2" /> Tambah Artikel</Button>
      </header>

      <div className="relative">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Cari judul..."
      ...
      <div className="overflow-x-auto rounded-2xl border border-border bg-card">
        <table className="w-full text-sm">
          ...
          <tbody>
            {isLoading && <tr><td colSpan={5} className="text-center py-10 text-muted-foreground">Memuat...</td></tr>}
            {!isLoading && filtered.map((a) => (
              <tr key={a.id} className="border-t border-border hover:bg-accent/40">
                <td className="px-4 py-3">
                  {a.cover_url ? (
                    <img src={a.cover_url} alt={a.title} className="h-10 w-14 object-cover rounded-lg border border-border" />
                  ) : (
                    <div className="h-10 w-14 rounded-lg border border-border bg-muted grid place-items-center text-[10px] text-muted-foreground">No img</div>
                  )}
                </td>
                ...
                <td className="px-4 py-3 hidden md:table-cell tabular-nums text-muted-foreground">{new Date(a.published_at).toLocaleDateString("id-ID")}</td>
                <td className="px-4 py-3 text-right">
                  <div className="inline-flex gap-1">
                    <Button size="sm" variant="ghost" onClick={() => openEdit(a)}><Pencil className="h-3.5 w-3.5" /></Button>
                    <Button size="sm" variant="ghost" onClick={() => remove(a.id)} className="text-destructive hover:text-destructive"><Trash2 className="h-3.5 w-3.5" /></Button>
                  </div>
                </td>
              </tr>
            ))}
            {!isLoading && filtered.length === 0 && (
              <tr><td colSpan={5} className="text-center py-10 text-muted-foreground">Tidak ada artikel.</td></tr>
            )}
          </tbody>
        </table>
      </div>
