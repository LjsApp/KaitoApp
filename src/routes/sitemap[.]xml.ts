  ...
  server: {
    handlers: {
      GET: async () => {
        ...
        if (categories) {
          for (const c of categories) entries.push({ path: `/kategori/${c.slug}`, changefreq: "weekly", priority: "0.7" });
        }
        if (products) {
          for (const p of products) entries.push({ path: `/produk/${p.slug}`, changefreq: "weekly", priority: "0.8" });
        }
        if (articles) {
          for (const a of articles) entries.push({ path: `/artikel/${a.slug}`, changefreq: "monthly", priority: "0.6" });
        }
