      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
        <div>
          <h1 className="font-display font-black text-3xl md:text-4xl tracking-tight">
            KTH <span className="text-primary">Edukasi</span>
          </h1>
          <p className="text-muted-foreground mt-2 max-w-2xl text-base">
            Wawasan ahli, tips perawatan, dan panduan seputar pompa air premium.
          </p>
        </div>
        ...
        <>
          {featured && (
            <div className="grid lg:grid-cols-12 gap-8 mb-16">
              <Reveal
                delay={0.1}
                className="lg:col-span-8 group relative rounded-2xl overflow-hidden aspect-[16/10] block"
              >
                <Link to="/artikel/$slug" params={{ slug: featured.slug }}>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-8">
                    <span className="rounded-full bg-accent-orange text-white px-3 py-1 text-xs font-bold uppercase tracking-widest">
                      {featured.category}
                    </span>
                    <h2 className="font-display font-black text-3xl md:text-4xl text-white mt-4 leading-tight line-clamp-2">
                      {featured.title}
                    </h2>
                ...
                {sideArticles.map((a, i) => (
                  <Reveal key={a.slug} delay={0.2 + i * 0.1}>
                    <Link
                      to="/artikel/$slug"
                      params={{ slug: a.slug }}
                      className="group flex gap-5 bg-card rounded-2xl overflow-hidden shadow-sm hover:shadow-elegant transition-all p-3 border border-border/50"
                    >
                      ...
                      <div className="flex flex-col justify-center flex-1 py-1 pr-2">
                        <span className="text-accent-orange font-bold text-[10px] mb-2 uppercase tracking-widest">
                          {a.category}
                        </span>
                        <h3 className="font-display font-bold text-sm md:text-base line-clamp-2 group-hover:text-primary transition-colors leading-snug">
                          {a.title}
                        </h3>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4 text-xs text-muted-foreground mb-3">
                        <span className="flex items-center gap-1.5">
                          <Calendar className="w-3.5 h-3.5" /> {formatDate(a.published_at)}
                        </span>
                      </div>
                      <h4 className="font-display font-bold text-xl group-hover:text-primary transition-colors line-clamp-2 mb-3">
                        {a.title}
                      </h4>
                      <p className="text-muted-foreground text-sm line-clamp-3 mb-4 flex-1">
                        {a.excerpt}
                      </p>
                      <span className="mt-auto inline-flex items-center gap-1 text-primary text-sm font-semibold">
