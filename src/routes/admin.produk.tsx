...
export function Modal({
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-foreground/50 backdrop-blur-sm grid place-items-center p-4 overflow-y-auto">
      <div
        className={`bg-card rounded-2xl border border-border shadow-elegant ${wide ? "max-w-4xl" : "max-w-3xl"} w-full my-8 max-h-[92vh] overflow-y-auto`}
      >
        ...
        <div className="rounded-xl border border-border overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-muted/50 text-xs uppercase tracking-wider text-muted-foreground">
              <tr>
                <th className="text-left px-4 py-2">Nama Keunggulan</th>
...
function ProductModal({ initial, onClose }: { initial: ProdForm; onClose: () => void }) {
  ...
  const set = useCallback(
    <K extends keyof ProdForm>(k: K, v: ProdForm[K]) => setForm((f) => ({ ...f, [k]: v })),
    [],
  );
  ...
  const removeSpec = (i: number) =>
    set(
      "specs",
      form.specs.filter((_, j) => j !== i),
    );
  const updateSpec = (i: number, field: "key" | "value", v: string) =>
    set(
      "specs",
      form.specs.map((s, j) => (j === i ? { ...s, [field]: v } : s)),
    );

  const toggleFeature = (id: string) =>
    set(
      "feature_ids",
      form.feature_ids.includes(id)
        ? form.feature_ids.filter((f) => f !== id)
        : [...form.feature_ids, id],
    );
          ...
          <section>
            <div className="flex items-center justify-between mb-4">
              ...
              <Button type="button" size="sm" variant="outline" onClick={addSpec}>
                <Plus className="h-3.5 w-3.5 mr-1" /> Tambah Spesifikasi
              </Button>
            </div>
            {form.specs.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-4 border border-dashed border-border rounded-xl">
                Belum ada spesifikasi. Klik "Tambah" untuk menambah baris.
              </p>
            ) : (
                ...
                {form.specs.map((s, i) => (
                  <div key={i} className="flex gap-2 items-center">
                    ...
                    <input
                      ...
                      placeholder="Nama (mis: Daya)"
                      className="flex-1 h-10 px-3 rounded-lg bg-muted border border-border outline-none text-sm focus:ring-2 focus:ring-primary/40"
                    />
                    <input
                      value={s.value}
                      onChange={(e) => updateSpec(i, "value", e.target.value)}
                      placeholder="Nilai (mis: 125 Watt)"
                      className="flex-1 h-10 px-3 rounded-lg bg-muted border border-border outline-none text-sm focus:ring-2 focus:ring-primary/40"
                    />
                    <button
                      type="button"
                      onClick={() => removeSpec(i)}
                      className="grid h-9 w-9 shrink-0 place-items-center rounded-lg hover:bg-destructive/10 text-destructive transition-colors"
                    >
          ...
          <section>
            ...
            {features.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                Belum ada keunggulan. Klik "Kelola" untuk menambah.
              </p>
            ) : (
              <div className="grid sm:grid-cols-2 gap-2">
                {features.map((f) => (
                  <label
                    key={f.id}
                    className="flex items-center gap-2.5 cursor-pointer p-2.5 rounded-lg hover:bg-accent/50 transition-colors"
                  >
                    <input
                      type="checkbox"
          ...
          <section>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <Input
                ...
                onChange={(v) => set("shopee_url", v)}
                placeholder="https://shopee.co.id/..."
              />
              <Input
                label="Link Tokopedia"
                value={form.tokopedia_url}
                onChange={(v) => set("tokopedia_url", v)}
                ...
                >
                  <td className="px-4 py-3">
                    {p.gallery?.[0] ? (
                      ...
                      <div className="h-10 w-10 rounded-lg border border-border bg-muted grid place-items-center">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${p.featured ? "bg-accent-orange/15 text-accent-orange" : "bg-muted text-muted-foreground"}`}
                    >
