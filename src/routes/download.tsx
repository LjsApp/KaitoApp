  ...
  head: () => ({
    meta: [
      { title: "Download Center — Kaito Hiro" },
      {
        name: "description",
        content:
          "Unduh brosur, manual book, datasheet, katalog, dan sertifikat resmi pompa air KTH.",
      },
          ...
          {isLoading ? (
            ...
            list.map((d, i) => (
              <Reveal key={d.id} delay={Math.min(i * 0.05, 0.4)}>
                <Card className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 hover-lift">
